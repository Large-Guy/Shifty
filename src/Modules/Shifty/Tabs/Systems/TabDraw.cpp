...
void TabDraw::Command::execute(SDL_Renderer* renderer)
{
    ...
    if (Entity::find<Focus>()->focused != Entity::null &&
        Entity::find<Focus>()->focused.has<Pane>() &&
        Entity::find<Focus>()->focused.get<Hierarchy>()->children.front() == tab.owner)
    {
        ComRef<Hierarchy> hierarchy = Entity(tab.owner).get<Hierarchy>();
        auto animation = Entity(hierarchy->parent).get<Animation>();
        ...
        [draw](ComRef<RenderTransform> transform, ComRef<Pane> view, ComRef<Hierarchy> hierarchy)
        {
            ...
            for (int i = 0; i < hierarchy->children.size(); ++i)
            {
                draw.draw->pushCommand(
                    std::make_shared<Command>(hierarchy->children[i].get<RenderTransform>(),
                                              hierarchy->children[i].get<Motion>(),
                                              hierarchy->children[i].get<Tab>(), i));
            }
        });
}
